...
async def _enforce_wing_cross_guard(
    *,
    db_session: Any,
    delta_client: Any,
    trade: Trade,
    ...
    """
    Keep the rolled short STRICTLY inside the open wing on that side.

    call: new_strike < wing_call
    put:  new_strike > wing_put

    Returns AdjustmentResult to abort (trade stays ACTIVE, no orders) when
    there is no room inside the wing. Returns None when OK / no wing / clamped
    in place (plan mutated).
    """
    ...
    side = str(triggered_leg_type or "").lower().strip()
    if side not in ("call", "put"):
        return None

    wing_leg_type = "wing_call" if side == "call" else "wing_put"
    wing_leg = (
        db_session.query(Leg)
        .filter(
            Leg.trade_id == int(trade.id),
            Leg.leg_type == wing_leg_type,
            Leg.status == "open",
        )
        .first()
    )
    if wing_leg is None:
        # Wings off or already closed — 2-leg strangle path unchanged
        return None

    try:
        wing_strike = float(getattr(wing_leg, "strike", 0) or 0)
    except (TypeError, ValueError):
        wing_strike = 0.0
    if wing_strike <= 0:
        return None

    wanted = float(plan.new_strike)
    current = float(triggered_leg.strike)
    crosses = (
        wanted >= wing_strike if side == "call" else wanted <= wing_strike
    )
    if not crosses:
        return None

    und = str(getattr(trade, "underlying", None) or "BTC").upper()
    ...
    try:
        chain = await delta_client.get_option_chain(und, expiry_str)
    except Exception as exc:
        logger.critical(
            "[WING_CROSS_GUARD_ABORT] trade=%s could not load chain: %s",
            trade.id,
            exc,
        )
        log_and_buffer(
        ...
        return AdjustmentResult(
            ...
            new_strike=wanted,
            error_message=(
                f"WING_CROSS_GUARD_ABORT: chain_fetch_failed for {side}"
            ),
        )
    ...
    clamped, status = clamp_short_strike_inside_wing(
        ...
        current_short_strike=current,
    )

    if status == "dead_end" or clamped is None:
        log_and_buffer(
            "WING_CROSS_GUARD_ABORT",
            int(trade.id),
            {
        ...
        logger.critical(
            "[WING_CROSS_GUARD_ABORT] trade=%s leg=%s wanted=%s wing=%s "
            "current=%s reason=no_room_inside_wing — abort before close",
            trade.id,
            side,
            wanted,
            wing_strike,
        ...
        return AdjustmentResult(
            ...
            new_strike=wanted,
            error_message=(
                f"WING_CROSS_GUARD_ABORT: no_room_inside_wing leg={side} "
                f"wanted={wanted} wing={wing_strike}"
            ),
        )
    ...
    crow = find_chain_row_for_strike(
        chain or [], leg=side, strike=float(clamped)
    )
    if crow is None:
        log_and_buffer(
            "WING_CROSS_GUARD_ABORT",
            int(trade.id),
            {
                ...
                "wing_strike": wing_strike,
                "current_strike": current,
                "clamped_to": float(clamped),
                "reason": "clamped_strike_missing_from_chain",
            },
        )
        logger.critical(
            "[WING_CROSS_GUARD_ABORT] trade=%s clamped=%s missing from chain",
            trade.id,
            clamped,
        )
        return AdjustmentResult(
            ...
            new_strike=wanted,
            error_message=(
                f"WING_CROSS_GUARD_ABORT: clamped strike {clamped} "
                f"missing from chain"
            ),
        )

    mark_key = "call_mark_price" if side == "call" else "put_mark_price"
    clamped_premium = float(crow.get(mark_key) or 0)
    if side == "call":
        new_symbol = str(crow.get("call_symbol") or "")
        new_product_id = int(crow.get("call_product_id") or 0)
    else:
        new_symbol = str(crow.get("put_symbol") or "")
        new_product_id = int(crow.get("put_product_id") or 0)

    plan.new_strike = float(clamped)
    plan.new_product_id = new_product_id
    plan.new_symbol = new_symbol
    ...
    log_and_buffer(
        ...
        int(trade.id),
        {
            ...
            "clamped_premium": round(clamped_premium, 4),
        },
    )
    logger.warning(
        "[WING_CROSS_GUARD] trade=%s leg=%s wanted=%s wing=%s clamped_to=%s "
        "target_prem=%.4f clamped_prem=%.4f",
        trade.id,
        side,
        wanted,
        wing_strike,
    ...
    # Tolerance bypass: still proceed with clamped strike (structure > premium)
    tolerance_pct = 40.0
    try:
        ...
        with SessionLocal() as _sdb:
            _cfg = _sdb.query(AutoTradeSettings).first()
            if _cfg is not None:
                tolerance_pct = float(
                    getattr(_cfg, "entry_premium_match_tolerance_pct", None)
                    or getattr(_cfg, "adjustment_premium_tolerance_pct", None)
                    or 40.0
                )
    except Exception:
        tolerance_pct = 40.0
    ...
    deviation_pct = (
        abs(clamped_premium - float(target_premium))
        / float(target_premium)
        * 100.0
        if float(target_premium) > 0
        else 0.0
    )
    if deviation_pct > tolerance_pct:
        ...
        logger.warning(
            "[WING_CROSS_GUARD_TOLERANCE_BYPASS] trade=%s leg=%s "
            "clamped_to=%s target=%.4f clamped_prem=%.4f deviation=%.2f%%",
            trade.id,
            side,
            clamped,
            float(target_premium),
...
class AdjustmentExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            # Wing cross-guard BEFORE any order (close or entry).
            # find_adjustment_strike may already clamp; this is the last
            # hard gate so a crossed short never reaches the close step.
            wing_abort = await _enforce_wing_cross_guard(
