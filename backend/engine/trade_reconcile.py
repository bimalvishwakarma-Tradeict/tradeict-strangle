...
from backend.core.realized_booking import (
    basket_realized_breakdown,
    book_leg_close as _book_leg_close_core,
    pnl_sanity_check,
...
def book_leg_close(
    *,
    ...
    When ``db`` is provided, recomputes ``trade.realized_pnl`` immediately
    after booking (idempotent single source of truth).
    """
...
def recompute_trade_realized_pnl(db: Any, trade: Any) -> float:
    """Load bot-managed legs from DB and recompute trade.realized_pnl."""
    trade_id = int(getattr(trade, "id", 0) or 0)
    legs = (
        db.query(Leg)
        .filter(
    ...
    return recompute_trade_realized_from_legs(legs, trade)
